...
public class mxStackLayout extends mxGraphLayout
{
	...
	public void execute(Object parent)
	{
		if (parent != null)
		{
			mxIGraphModel model = model(parent);
			try
			{
			}
			finally
	...
	private mxIGraphModel model(Object parent) {
		boolean horizontal = isHorizontal();
		mxIGraphModel model = graph.getModel();
		...
		pgeo(parent, horizontal, model, pgeo);
		return model;
	}

	private void pgeo(Object parent, boolean horizontal, mxIGraphModel model, mxGeometry pgeo) {
		double fillValue = 0;
		if (pgeo != null) {
			fillValue = (horizontal) ? pgeo.getHeight() : pgeo.getWidth();
		}
		modelFillValue(parent, horizontal, model, pgeo, fillValue);
	}

	private void modelFillValue(Object parent, boolean horizontal, mxIGraphModel model, mxGeometry pgeo,
			double fillValue) {
		...
		int childCount = model.getChildCount(parent);
		pgeoChild(parent, horizontal, model, pgeo, fillValue, x0, y0, tmp, last, childCount);
	}

	private void pgeoChild(Object parent, boolean horizontal, mxIGraphModel model, mxGeometry pgeo, double fillValue,
			double x0, double y0, double tmp, mxGeometry last, int childCount) {
